...
with open("../generated_txt/cisse_lexicon.txt", "w", encoding="utf-8") as txtfile:
    # Extract and write the values of the <mot_vedette> tag to the text file
    for article in root.findall("article"):
        mot_vedette_element = article.find("bloc_forme/mot_vedette")
        mot_vedette = mot_vedette_element.text if mot_vedette_element is not None else ""
        txtfile.write(mot_vedette + "\n")

print("Text file with mot_vedette values has been generated.")
